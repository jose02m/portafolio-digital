...
import { FaArrowRight, FaCircleCheck, FaCircleInfo, FaCodeBranch, FaEye, FaRobot, FaXmark } from "react-icons/fa6";
...
export default function ProjectGallery() {
  ...
  return <section id="proyectos" className="projects section-pad section-shell">
    ...
    <div className="project-grid">{visible.map(project => <article className={project.featured ? "project-card featured" : "project-card"} key={project.title} data-reveal="up">
      {project.featured ? <EcosystemVisual/> : <ProjectVisual project={project}/>}<div className="project-content">{project.featured && <p className="project-label">Proyecto destacado</p>}<p className="project-category">{project.disciplines}</p><h3>{project.title}</h3><p>{project.description}</p><small>{project.result}</small><div className="project-actions"><button className="info-action" type="button" onClick={() => setSelected(project)} aria-label={`Ver información de ${project.title}`}><FaCircleInfo/><span>{project.featured ? "Ver caso de estudio" : "Información"}</span></button>{project.url ? <a className="demo-action" href={project.url} target="_blank" rel="noopener noreferrer" aria-label={`Abrir demo de ${project.title} en una pestaña nueva`}><FaEye/><span>Ver demo</span></a> : <button type="button" className="demo-action" onClick={() => showDemoNotice(project.title)} aria-label={`Consultar demo de ${project.title}`}><FaEye/><span>Demo próximamente</span></button>}</div></div>
    </article>)}</div>
...
function ProjectLinks({ project, fallback }: { project: Project; fallback: () => void }) { return <div className="modal-project-links">{project.url ? <a className="modal-demo-action" href={project.url} target="_blank" rel="noopener noreferrer"><FaEye/> Ver demo</a> : <button className="modal-demo-action" type="button" onClick={fallback}><FaEye/> Demo próximamente</button>}{project.repositoryUrl ? <a className="modal-repository-action" href={project.repositoryUrl} target="_blank" rel="noopener noreferrer"><FaCodeBranch/> Ver repositorio</a> : null}</div>; }
